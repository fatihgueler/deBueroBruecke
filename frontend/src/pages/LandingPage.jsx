...
import {
  ...
  Upload, Brain, FileCheck, Building2, Clock, FileText,
} from 'lucide-react';
        ...
        <div className="mt-4 space-y-2">
          {[1, 2, 3].map((n) => (
            <div key={n} className="flex items-center gap-2">
              <span className="h-5 w-5 rounded-full bg-primary-500/20 text-primary-400 text-xs flex items-center justify-center flex-shrink-0 font-bold">
                {n}
              </span>
  ...
  return (
    <main className="flex-1">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 -z-10 h-64 w-64 rounded-full bg-primary-900/20 blur-[80px]" />
        ...
        <div className="section-container py-10">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {LANGUAGES.map((lang) => (
              <div
                key={lang.code}
                className="flex items-center gap-2.5 rounded-xl border border-border bg-card px-4 py-2.5 text-sm font-medium text-slate-200 transition hover:border-primary-500/40 hover:bg-card-2"
              >
          ...
          {STEPS.map(({ icon: Icon, step, key }, idx) => (
            <div key={key} className="relative text-center">
              ...
              <div className="relative mx-auto mb-5 inline-flex h-16 w-16 items-center justify-center rounded-2xl border border-primary-500/25 bg-primary-500/10">
                ...
                <span className="absolute -right-2 -top-2 flex h-6 w-6 items-center justify-center rounded-full bg-primary-500 text-xs font-bold text-white shadow-glow">
                  {step}
                </span>
              </div>
        ...
        <div className="section-container py-12">
          ...
          <div className="flex flex-wrap justify-center gap-2.5">
            {AUTHORITY_TYPES.map((auth) => (
              <span
                key={auth}
                className="rounded-lg border border-border bg-card px-4 py-2 text-sm text-slate-300 transition hover:border-primary-500/30 hover:text-slate-100"
              >
                {auth}
              </span>
            ))}
        ...
        <div className="relative overflow-hidden">
          ...
          <div className="section-container py-24 text-center">
            ...
            <div className="mt-10 flex flex-wrap justify-center gap-4">
              ...
              {!isAuthenticated && (
                <Link to="/about" className="btn-secondary px-6 py-3.5 text-base">
                  {t('landing.learnMore')}
                </Link>
              )}
            </div>
